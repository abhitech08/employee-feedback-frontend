...
const UserModal = ({ user, companies = [], departments = [], error = '', onClose, onSubmit, loading }) => {
  const { register, handleSubmit, watch, setValue, formState: { errors } } = useForm({ defaultValues: { role: 'company_admin', status: 'active' } });
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg p-5 sm:p-6 max-w-2xl w-full max-h-[90vh] overflow-auto">
        ...
        <form className="grid grid-cols-1 md:grid-cols-2 gap-4" onSubmit={handleSubmit(onSubmit)}>
          ...
          <div>
            <label className="block text-gray-700 font-semibold mb-2">Password {user ? '(optional)' : ''}</label>
            <input
              type="password"
